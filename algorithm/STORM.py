...
def main(config):
    ...
    for x, y in tqdm(iterator.iter(data, label)):
        ...
        all_scores.append(-score)  # 使用与AUROC相同的分数（取负）
        all_labels.append(y)
    ...
    predictions = (sliced_scores > -0.5).astype(int)
...
def run_storm_detector(data, label, config_args):
    """
    封装 STORM (ExactStorm) 的运行过程
    """
    from pysad.models import ExactStorm
    from pysad.utils import ArrayStreamer
    import numpy as np
